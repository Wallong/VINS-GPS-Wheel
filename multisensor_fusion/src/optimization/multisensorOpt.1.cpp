...
void MultisensorOptimization::inputOdom(double t, Eigen::Vector3d odomP, Eigen::Quaterniond odomQ)
{
    ...
    poseStamped.pose.position.x = lastP.x();
    poseStamped.pose.position.y = lastP.y();
    poseStamped.pose.position.z = lastP.z();
    poseStamped.pose.orientation.x = lastQ.x();
    poseStamped.pose.orientation.y = lastQ.y();
    poseStamped.pose.orientation.z = lastQ.z();
    poseStamped.pose.orientation.w = lastQ.w();

    printf("Original: (x, y, z) = (%f, %f, %f), (w, x, y, z) = (%f, %f, %f, %f)\n", odomP.x(), odomP.y(), odomP.z(),
             odomQ.w(), odomQ.x(), odomQ.y(), odomQ.z());
    printf("GPS: (x, y, z) = (%f, %f, %f), (w, x, y, z) = (%f, %f, %f, %f)\n", lastP.x(), lastP.y(), lastP.z(),
             lastQ.w(), lastQ.x(), lastQ.y(), lastQ.z());
    ...
    while (true)
    {
        if (newGPS)
        {
            ...
            for (int i = 0; i < length; i++, iter++)
            {
                ...
                problem.AddParameterBlock(q_array[i], 4, localParameterization);
            ...
            for (iterVIO = localPoseMap.begin(); iterVIO != localPoseMap.end(); iterVIO++, i++)
            {
                ...
                if (iterVIONext != localPoseMap.end())
                {
                    ...
                    wTj.block<3, 1>(0, 3) = Eigen::Vector3d(iterVIO->second[0], iterVIO->second[1], iterVIO->second[2]);
                    ...
                    ceres::CostFunction* vioFunction = RelativeRTError::Create(iPj.x(), iPj.y(), iPj.z(),
                                                                                iQj.w(), iQj.x(), iQj.y(), iQj.z(),
                                                                                0.1, 0.01);
                    problem.AddResidualBlock(vioFunction, NULL, q_array[i], t_array[i], q_array[i+1], t_array[i+1]);
                    /*
